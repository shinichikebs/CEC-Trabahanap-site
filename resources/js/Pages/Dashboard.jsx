...
export default function Dashboard({ auth }) {
    ...
    useEffect(() => {
        if (selectedCategories.length > 0) {
            setFilteredData(
                data.filter((job) => selectedCategories.includes(job.category))
            );
        } else {
    ...
    const fetchData = () => {
        axios
            ...
            .then((response) => {
                ...
                setCategories([
                    ...new Set(
                        response.data.jobOffers.map((job) => job.category)
                    ),
                ]);
            })
            .catch((error) => {
            ...
            <div className="p-4 flex flex-col lg:flex-row">
                ...
                <div className="flex-grow p-4">
                    {filteredData
                        ...
                        ) // Sort in descending order based on created_at
                        .map((item) => (
                            <div
                                key={item.id}
...
{click && (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-2">
        {/* Desktop View */}
        <div className="hidden lg:block bg-white w-full max-w-5xl h-5/6 overflow-y-auto p-6 rounded-lg shadow-lg relative">
            <IoMdArrowBack
            ...
            <div className="mt-6">
                ...
                />
                <h2 className="font-medium text-gray-700 mt-3">Submit Proposal Documents (e.g., Portfolio):</h2>
                <p className="text-sm text-yellow-500 mt-1">
                    Warning: Do not submit documents containing confidential information.
                </p>
                <input type="file" className="mt-2" onChange={handleFileChange} ref={fileInputRef} />
            </div>
        ...
        {/* Mobile View */}
        <div className="lg:hidden bg-white w-full h-3/4 overflow-y-auto p-4 rounded-lg shadow-lg relative">
            <IoMdArrowBack
                className="absolute top-4 left-4 cursor-pointer text-gray-500"
                size={24}
                onClick={() => setClick(false)}
            />
            <h1 className="text-2xl font-semibold mt-10 mb-5">{details.job_title}</h1>
            {/* Repeat same content as desktop */}
            <div className="mb-4">
                <h2 className="font-semibold text-gray-700">Description:</h2>
                <p className="text-gray-600">{details.job_description || "No description provided."}</p>
            </div>
